...
def generate_morning_report() -> str:
    ...
    ai_text = _ai_synthesize(
        "盤前分析",
        "這是盤前 07:00 報告。嚴格依照以下五個區塊輸出，每個區塊一行標題：\n"
        "【環境評分】給出一個 -100 到 +100 的數字 + 一句話判斷\n"
        "【今日關鍵訊號】列出最重要的 3 條訊號（附數據）\n"
        "【操作方向】明確寫「偏多」「偏空」或「觀望」+ 理由\n"
        "【關鍵支撐壓力】列出支撐和壓力的具體數字\n"
        "【風險提示】今日需要注意的 1-2 個風險\n"
        "500 字以內，不要寫超出以上格式的內容。",
        combined,
    )
...
def generate_evening_report() -> str:
    ...
    ai_text = _ai_synthesize(
        "盤後分析",
        "這是盤後 18:00 報告。嚴格依照以下五個區塊輸出，每個區塊一行標題：\n"
        "【今日盤勢】收盤價 + 漲跌幅 + 一句話總結\n"
        "【籌碼解讀】外資/投信/散戶今日動向 + 期貨部位變化\n"
        "【選擇權壓力支撐】最大 Call OI 壓力 + 最大 Put OI 支撐 + 預估區間\n"
        "【明日操作建議】明確寫方向 + 進場區間 + 止損位\n"
        "【風險提示】需要注意的 1-2 個風險\n"
        "500 字以內，不要寫超出以上格式的內容。",
        combined,
    )
