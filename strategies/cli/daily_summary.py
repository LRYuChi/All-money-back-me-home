...
def _send_to_telegram(text: str) -> bool:
    """Reuse SupertrendStrategy's _send_to_all_bots so the message lands
    in the same bots ops already monitors. Falls back to stdout on any
    failure so this never crashes the cron."""
    try:
