...
| **G. 風險統一** | ✅ basic | GuardPipeline + 9 guards ✅；Worker 接 pipeline ✅；PnL aggregator × 4 ✅；ExposureProvider × 4 (+ open_by_symbol) ✅；G9 ConsecutiveLossDays ✅；SignalAgeProvider × 4 → G1 真實啟用 ✅；side-effect 鏈 (G9 disabler + notifier alert) ✅；G7 CorrelationCapGuard + CorrelationMatrix × 3 ✅；**G10 KellyPositionGuard (fractional Kelly with safety_factor，insufficient sample → ALLOW，negative-edge → DENY，by-symbol opt-in) + WinRateProvider × 3 (NoOp/InMemory/Postgres，跨 sm_paper_trades+live_trades 聚合) + WinRateStats dataclass + cli `--kelly-safety-factor/--kelly-min-trades/--kelly-lookback-days` ✅**；G2 ⬜ | 9/10 guards 完整；G2 待 F.1 exchange registry |
| **H. Live ramp** | ⬜ | — | — |
...
| — | — | **下輪待辦**：G2 SymbolSupported (依 F.1 exchange registry) OR OKX adapter scaffolding (F.1 第一步) OR daemon 接 refresh_strategies OR Audit log hook (round 7 leftover) OR multi-strategy daemon loader | ⬜ |
