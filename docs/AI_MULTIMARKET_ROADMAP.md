...
| **F. 跨市場** | 🟡 | Pending Orders middleware ✅；Worker (LogOnly + CLI + async run_forever) ✅；DispatcherRegistry + NotifyOnlyDispatcher ✅；execution/exchanges/ scaffolding + OKX adapter package ✅；**EventLogger × 4 (NoOp/InMemory/Postgres/Supabase) hooked into queue.enqueue/claim/update_status；fire-and-forget on backend failure；end-to-end Postgres 驗證 captures 3 transitions ✅**；OKX 真實 ccxt wiring / IBKR / TW broker ⬜ | pending_order_events 真有 writer，ops debugging audit trail 上線 |
...
| — | — | **下輪待辦**：pending_orders timeout/expired (避免卡死 PENDING) OR ccxt-okx 真實 wiring (有 demo key 可動) OR worker SUBMITTED-state polling (live mode 真用得上) OR signal-health API (P4 Gate 驗收的工具) | ⬜ |
