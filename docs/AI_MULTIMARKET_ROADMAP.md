...
| **F. 跨市場** | ✅ basic | Pending Orders middleware ✅；Worker (LogOnly + CLI + async run_forever) ✅；DispatcherRegistry + NotifyOnlyDispatcher ✅；execution/exchanges/ scaffolding + OKX adapter package ✅；EventLogger × 4 hooked into queue + fire-and-forget ✅；sweep_expired() on all 4 backends + cli/sweep.py + 自動寫 EXPIRED audit event ✅；CcxtOKXClient 接通 ccxt-okx (place + cancel + fetch_instruments + fetch_order)；build_default_registry 自動 wire live/paper；real demo OKX 已驗證；**OKXLiveDispatcher.fetch_status() + background_poll_submitted_loop() sidecar (close ACCEPTED→FILLED 半完成單迴路)**；IBKR / TW broker ⬜ | OKX 真實 trade-and-track loop 全鏈完成 (demo)；live 只差換正式 API key |
| **G. 風險統一** | ✅ | GuardPipeline + 10 guards ✅；Worker 接 pipeline ✅；PnL aggregator × 4 ✅；ExposureProvider × 4 (+ open_by_symbol) ✅；G9 ConsecutiveLossDays ✅；G1 真實啟用 ✅；side-effect 鏈 ✅；G7 CorrelationCap + Matrix × 3 ✅；G10 Kelly + WinRateProvider × 3 ✅；**G2 SymbolSupportedGuard (catalog-driven，NoOp 自動 fail-open，lookup 失敗 fail-open) + SymbolCatalog 抽象 (NoOp/InMemory/CachedSymbolCatalog with TTL + 第一次載入失敗 fail-CLOSED) ✅**| **10/10 guards 完整** |
| **H. Live ramp** | ⬜ | — | — |
...
| (manual) | #30 | Phase G — G10 KellyPositionGuard (fractional Kelly + safety_factor + insufficient sample fail-open + negative-edge DENY + by-symbol opt-in) + WinRateProvider × 3 (NoOp/InMemory specific>broad fallback/Postgres GROUP BY 聚合) + WinRateStats dataclass with kelly_fraction property + cli `--kelly-safety-factor/--kelly-min-trades/--kelly-lookback-days` 32 tests，全 353 tests 綠 | ✅ 完成 |
...
| (loop #2) | #36 | pending_order_events writer — execution/pending_orders/events.py with EventLogger Protocol + 4 backends (NoOp/InMemory/Postgres/Supabase)；wired into all 3 queue backends (enqueue→PENDING、claim→DISPATCHING、update_status→terminal)；fire-and-forget on backend failure (audit gap > blocking trades)；build_queue 自動 wire matching event_logger；Postgres uses xmax=0 to skip event on idempotent enqueue retry；end-to-end Postgres smoke captures 3 transitions 23 tests，全 1358 tests 綠 | ✅ 完成 |
| (loop #3) | #37 | pending_orders timeout sweeper — sweep_expired() on all 4 backends (Protocol + NoOp/InMemory/Postgres/Supabase) with separate pending/dispatching thresholds (0 = bucket disabled)；reasons captured per-row ("pending Xs ≥ Ys threshold" / "dispatching ... worker likely crashed")；自動觸發 round 36 events writer 寫 EXPIRED 轉換；個別 row 失敗不終止 batch sweep；新 cli/sweep.py 支援 one-shot or loop with --interval；end-to-end Postgres backdating smoke 通過 18 tests，全 1376 tests 綠 | ✅ 完成 |
| (loop #4) | #38 | worker CLI 內建 background sweeper — 新 sweep_runner.py 提供 background_sweep_loop() helper (testable in isolation 不綁 CLI) + SweepStats dataclass；wired into work.py 的 run_worker (`--sweep-interval-sec/--sweep-pending-max-age/--sweep-dispatching-max-age` flags)；shared graceful shutdown via stop_event；asyncio.wait_for(stop_event.wait(), timeout) idiom 讓 sweeper 立即響應 SIGINT；最終 stats 記入 worker shutdown log；單一 CLI invocation 同時跑 dispatch + sweep，省 cron 11 tests，全 1387 tests 綠 | ✅ 完成 |
| (loop #5) | #39 | worker /healthz HTTP endpoint — 純 stdlib (asyncio.start_server + 自寫 HTTP/1.0 parser，無新 dep)；3 endpoints: GET /healthz (always 200，liveness)、GET /ready (200 or 503 based on queue probe，readiness)、GET /stats (worker.stats() + sweep stats + queue depth + uptime，scraper-friendly)；query string 容忍、unknown path 404、non-GET 405、worker stats exception 包入 JSON body 不爆 endpoint；worker CLI 加 `--healthz-port/--healthz-host` flags，shared shutdown via run_worker；15 tests (含 concurrent + clean shutdown verification)，全 1402 tests 綠 | ✅ 完成 |
| (loop #6) | #40 | /metrics Prometheus exposition — 新 GET /metrics endpoint with Prometheus 0.0.4 text format (Content-Type: text/plain; version=0.0.4)；10 worker counters (claimed/filled/rejected/cancelled/partially_filled/dispatcher_errors/guard_denies/guard_scales/side_effects_invoked/side_effect_errors) + 3 sweeper counters (iterations/total_expired/errors) + 2 gauges (worker_uptime_seconds + pending_orders_pending_depth) + 3 health gauges (worker_stats_ok / sweep_stats_ok / queue_probe_ok)；rollout-friendly: missing/unknown stat keys 略過 series 而非顯示 0；every series 都有 HELP + TYPE comment；probe failure 不爆 endpoint，反映在 ok=0 gauge；16 tests (含 format compliance + 部分 stat 漏 + probe failure)，全 1418 tests 綠 | ✅ 完成 |
| (loop #7) | #41 | ccxt-okx 真實 wiring — 新 CcxtOKXClient 完整實作 OKXClient Protocol (place_order/cancel_order/fetch_instruments)；symbol mapping helpers (canonical↔ccxt with idempotent passthrough)；notional→amount 自動換算 (limit 用 limit_price，market 用 fetch_ticker)；amount_to_precision tick rounding；set_sandbox_mode for demo；OKX V5 SWAP defaults tdMode=cross + clOrdId 在 params；ccxt 例外 mapping (NetworkError/Timeout→NETWORK_ERROR、duplicate→DUPLICATE、其他→REJECTED)；fetch_instruments filter to USDT-SWAP active；build_okx_dispatcher 接 CcxtOKXClient + credential store fallback；build_default_registry 自動 wire live/paper (creds 缺時 paper 退 LogOnly，live 不註冊強制 exit)；real demo OKX 端到端 fetch 120 instruments 通過 36 new tests，全 1454 tests 綠 | ✅ 完成 |
| (loop #8) | #42 | SUBMITTED-state polling — OKXClient Protocol 加 fetch_order(coid, symbol)；FakeOKXClient 接 fetch_responses；CcxtOKXClient 用 ccxt.fetch_order with clOrdId in params + OrderNotFound→REJECTED；OKXLiveDispatcher 加 fetch_status(order)→DispatchResult|None (None when no coid)；新 poll_runner.py with PollStats (iterations/orders_polled/orders_advanced/errors)；background_poll_submitted_loop 跑 list_recent(SUBMITTED) → fetch_status → update_status 鏈，per-order failure isolation；max_orders_per_iteration 防 batch 過大；ACCEPTED 維持 SUBMITTED (只在 differs 才 update)；worker CLI `--poll-submitted-interval-sec/--poll-max-orders` flags + shared graceful shutdown 14 tests，全 1468 tests 綠 | ✅ 完成 |
| (loop #9) | #43 | retry/backoff for OKX network errors — 新 execution/exchanges/retry.py with RetryPolicy dataclass (max_attempts/base_delay/max_delay/multiplier/jitter_pct，全 validation) + retry_with_backoff helper (sleep + rng injectable for tests)；類別名匹配 (NetworkError/RequestTimeout/DDoSProtection/ExchangeNotAvailable/ExchangeError/ConnectionError/TimeoutError) 安全列表，AuthenticationError/InvalidOrder/InsufficientFunds 等不重試；CcxtOKXClient 構造接 retry_policy 參數，place_order/fetch_order/cancel_order 全鏈走 retry；lazy per-call 包裝避免 partial-stub 構造失敗；conservative default (3 attempts/0.5s base/5s cap/±20% jitter)；24 retry + 2 ccxt integration tests，全 1494 tests 綠 | ✅ 完成 |
| (loop #10) | #44 | strategy intent dedup window — 修補 prod 重大缺口 (deterministic coid 用 intent_ts ms 計算 → 兩次近時間 intent 不同 coid → DB UNIQUE 不擋 → 雙開倉)；新 execution/pending_orders/dedup.py with IntentDeduper Protocol + 3 backends (NoOp / WindowedIntentDeduper in-memory thread-safe with LRU eviction at max_keys / QueueBasedIntentDeduper cross-process via list_recent)；DedupKey = (strategy_id, symbol, side)；不同 side (long↔short) 不算重複 (容許反向)；naive ts 自動 coerce UTC；window=0 完全停用；make_intent_callback 加 deduper= kwarg，dedup 失敗 fail-open，record() 只在 enqueue 成功後執行 (失敗 retry 不消耗 slot)；NEUTRAL 過濾在 dedup 之前；35 tests 含 e2e 兩次 100ms intent 只開一單，全 1529 tests 綠 | ✅ 完成 |
| (loop #11) | #45 | shadow daemon dedup wiring — 把 round 44 接到 prod；smart_money/cli/shadow.py 加 `--intent-dedup-window-sec` flag (default 60s — secure-by-default，覆蓋 WS reconnect / 訊號聚合雙計 / 鯨魚事件叢集)；新 _build_deduper(window_sec) factory (window>0 → Windowed，≤0 → NoOp，負值容忍視為停用)；_maybe_build_strategy_runtime 透過 factory 傳給 make_intent_callback；runtime startup log 補 dedup 類別+窗格資訊；12 tests (parser default/custom/zero/fractional + factory selection + window propagation + e2e parser→factory + default 啟用 dedup)，全 1541 tests 綠 | ✅ 完成 |
| (manual) | #46 | **SupertrendStrategy 功能完善** — 新 strategies/journal.py (TradeJournal JSONL writer + 7 dataclass event types: Entry/PartialExit/TrailingUpdate/Exit/CircuitBreaker/Skipped + MultiTfState + StoplossPlan + TakeProfitPlan + UTC date partition + 線程安全 + range/day reader)；新 strategies/performance.py (PerformanceAggregator 算 win_rate/profit_factor/expectancy/Kelly/Sharpe/max_drawdown/streaks + by_tag/by_pair/by_exit_reason 分群 + Markdown 格式器中文輸出)；新 strategies/cli/journal_report.py (--days/--trades/--from/--to + md/json/compact 輸出格式)；supertrend.py 全鏈接入：confirm_trade_entry 寫 EntryEvent (含完整 SL plan + TP plan + 多時框狀態 + Kelly/quality 倉位推理) + Telegram 訊息加豐富(鎖盈計畫/部分出場規劃/4 時框 ▲▼ 狀態)；custom_stoploss 用 trade.set_custom_data 追蹤 max_profit + trailing_phase + 階段轉換時寫 TrailingUpdateEvent；adjust_trade_position 寫 PartialExitEvent；circuit breaker trip 寫 CircuitBreakerEvent + Telegram 警報；confirm_trade_exit 寫 ExitEvent (含 max_profit + trailing_phase + entry_tag 用於 per-tag 統計)，Telegram 訊息加 "高點留 X% 在桌上"；49 tests (23 journal + 26 performance) 含執行緒安全 + e2e 完整鏈路驗證，全 1590 tests 綠 | ✅ 完成 |
| — | — | **下輪待辦**：daily_summary 自動 cron 每日打 Telegram + 寫 daily_summary event OR signal-health API OR worker CLI 暴露 retry policy flags OR pending_orders dashboard panel | ⬜ |
