...
| **F. 跨市場** | 🟡 | Pending Orders middleware ✅；Worker (LogOnly + CLI + async run_forever) ✅；DispatcherRegistry + NotifyOnlyDispatcher ✅；**execution/exchanges/ scaffolding (Protocol-based ExchangeRequest/Response，deterministic client_order_id，CachedSymbolCatalog) + OKX adapter package (OKXClient Protocol + FakeOKXClient + OKXLiveDispatcher 全 6 status mapping + OKXSymbolCatalog) ✅**；OKX 真實 ccxt wiring / IBKR / TW broker ⬜ | F.1 第一步完成；scaffolding 可立即接 ccxt.okx |
...
| — | — | **下輪待辦**：multi-strategy daemon loader OR Audit log hook (round 7 leftover) OR ccxt-okx 真實 wiring (需 OKX_API_KEY) OR daemon 接 refresh_strategies | ⬜ |
