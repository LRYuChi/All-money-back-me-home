...
| — | — | **下輪待辦**：ccxt-okx 真實 wiring (有 demo key 可動，需小心 mock 隔離) OR worker SUBMITTED-state polling (live mode 真用得上但 depends on ccxt) OR signal-health API (P4 Gate 工具) OR worker stats /healthz endpoint (ops monitoring) | ⬜ |
