...
| **F. 跨市場** | ✅ basic | Pending Orders middleware ✅；Worker (LogOnly + CLI + async run_forever) ✅；DispatcherRegistry + NotifyOnlyDispatcher ✅；execution/exchanges/ scaffolding + OKX adapter package ✅；EventLogger × 4 hooked into queue + fire-and-forget ✅；sweep_expired() on all 4 backends + cli/sweep.py + 自動寫 EXPIRED audit event ✅；**CcxtOKXClient 接通 ccxt-okx (place_order + cancel_order + fetch_instruments)，notional→amount 自動換算，set_sandbox_mode for demo，credentials 走 .env 或 credential store fallback；build_default_registry 自動 wire live/paper 到 OKX (creds 缺時 paper 退 LogOnly、live 不註冊)；real demo OKX 已驗證 fetch 120 USDT-SWAP perps**；IBKR / TW broker ⬜ | OKX 真實可下單 (demo)；live mode 需正式 API key |
...
| — | — | **下輪待辦**：worker SUBMITTED-state polling (live mode 真有 ACCEPTED 半完成單需要 poll fill) OR signal-health API OR pending_orders dashboard panel OR per-symbol size precision verifier (live demo trial run) | ⬜ |
