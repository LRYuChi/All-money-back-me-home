...
| **F. 跨市場** | 🟡 | Pending Orders middleware ✅；Worker (LogOnly + CLI + async run_forever) ✅；DispatcherRegistry + NotifyOnlyDispatcher ✅；execution/exchanges/ scaffolding + OKX adapter package ✅；EventLogger × 4 hooked into queue + fire-and-forget ✅；**sweep_expired() on all 4 backends + cli/sweep.py one-shot/loop CLI + 自動寫 EXPIRED audit event**；OKX 真實 ccxt wiring / IBKR / TW broker ⬜ | 防止 PENDING/DISPATCHING 卡死無限累積 |
...
| — | — | **下輪待辦**：ccxt-okx 真實 wiring (有 demo key 可動) OR signal-health API (P4 Gate 工具) OR pending_orders dashboard panel OR worker SUBMITTED-state polling (depends on ccxt) | ⬜ |
